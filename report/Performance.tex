...
On an embedded platform with limited \ca{computational} power, the loading time of \texttt{VixenConsole} and all controller modules can take a significant \cb{amount of} time. The \texttt{tidy} operation can reduce a small portion of the loading time, by remove unused settings for elements and filters\ca{, which} also reduces the size of XML \ca{configuration files}.
...
\fref{fig:perf_RPi} shows the performance comparison between different implementations on Raspberry Pi B. \texttt{VixenLinky} (referenced by \texttt{raw}) has the highest refresh rate for its simplicity. \texttt{VixenConsole} with the ``Raw'' sequence format (referenced by \texttt{seq}) has the second highest performance. \ca{With audio media file (referenced by \texttt{seqa}), the performance dropped negligibly.}

The unlimited playback performance drops to almost half when using a \texttt{rgb24} encoded video as the input (referenced by \texttt{v}). The performance drops again by a small factor when audio stream was also added to the video input (referenced by \texttt{va}). The video only playback performance of \texttt{yuv420p} encoding (referenced by \texttt{v420}) is only a little bit higher than the lossless \texttt{rgb24} encoding (\texttt{v}), whereas the performance of lossless \texttt{yuv444p} encoding (referenced by \texttt{vyuv}) is noticeably lower. Therefore, \ca{pixel format \texttt{rgb24}} is more suitable for encoding video sequences.

\ca{Performance comparisons on other platforms were also analysed. \fref{fig:perf_RPi3} and} \fref{fig:perf_Laptop} shows the performance comparison \ca{on two of them}, \ca{the Raspberry Pi 3B platform and} the \cb{same laptop running Linux}. \ca{The refresh rates easily reached a few hundreds of fps on the Raspberry Pi 3B platform, and more on the \cb{laptop}}. \ca{On the \cb{laptop}, the} maximum refresh rates \ca{reached} over 1,000 fps for playback and 10,000 fps for controllers. For the 50 fps \ca{test} sequence, the CPU usage almost never reaches above $2 \%$. \ca{Compared} to the original Vixen application struggling to \ca{reach a} 50 fps refresh rate on Microsoft Windows, \cb{this} new playback engine definitely achieved \ca{substantial} performance improvements.
...
\begin{figure*}[t]
  ...
  \caption{\footnotesize Performances of different video container formats}
  \label{fig:fmts}
\end{figure*}
...
\ca{
\fref{fig:fmts} shows performance companions of multiple lossless combinations of video container and encoding formats on these three platforms. \tref{tbl:fmts} lists their relevant file sizes. There is only one entry with QuickTime encoding, because the QuickTime encoder from \texttt{ffmpeg} only supports \texttt{mov} and \texttt{mkv} container formats, and the \texttt{mov} container gives smaller file size. Because \cb{the} \texttt{h265} encoding \cite{h265} \cb{was} not supported by \texttt{libav} on Raspberry Pi \cb{platforms}, and it did not show outstanding performance or file size optimisation on the \cb{laptop}, it was not tested on Raspberry Pi platforms.
