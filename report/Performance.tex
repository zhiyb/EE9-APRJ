...
\begin{figure*}[t]
  ...
  \label{fig:perf_RPi3}}
  \hfil
  \subfloat[Jetson TX2]{\includegraphics[width=0.6\textwidth]{Figs/types-TX2-perf.eps}%
  \label{fig:perf_TX2}}
...
\begin{figure*}[t]
  ...
  \label{fig:fmts_RPi3}}
  \hfil
  \subfloat[Jetson TX2]{\includegraphics[width=0.66\textwidth]{Figs/fmts-more-TX2-perf.eps}%
  \label{fig:fmts_TX2}}
...
\ca{
\fref{fig:fmts} shows performance companions of multiple lossless combinations of video container and encoding formats on these three platforms. \tref{tbl:fmts} lists their relevant file sizes. There is only one entry with QuickTime encoding, because the QuickTime encoder from \texttt{ffmpeg} only supports \texttt{mov} and \texttt{mkv} container formats, and the \texttt{mov} container gives smaller file size. Because \texttt{h265} decoding are not supported by \texttt{libav} on Raspberry Pis, and it did not show outstanding performance or file size optimisation on the Jetson TX2 platform, it was not tested on Raspberry Pi platforms.
